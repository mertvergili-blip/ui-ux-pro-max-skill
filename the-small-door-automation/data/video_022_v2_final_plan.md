...
Censored swears are always rendered censored on screen. Two different audio
strategies coexist now (see "Bleep/censor rules" below) — captions stay
censored in both cases, never spelled out in full.
...
("Breakfast is over, [bleep]."). They now use two different strategies:

- **Line 4 — audio uncensored, caption-only censor**: the user rejected the
  earlier "I'll [bleep] you up." beep-placeholder version as too artificial
  and not funny. The replacement line is sent to and spoken by ElevenLabs
  exactly as written — `"I'll fuck you up, mate."` — no beep placeholder, no
  censor tone applied in the mix. Only the **caption** is censored:
  `"I'll f*** you up, mate."`.
- **Line 7 — audio + caption censor (unchanged)**: the literal swear word is
  never sent to or generated by the TTS engine. `generate_voice_022.py`
  sends ElevenLabs a literal "beep" placeholder in that word's position
  ...
  Caption is rendered censored (`b****`), never spelled out in full, never
  ahead of or contradicting the audio beep.
...
- The superseded line-4 beep version (`04_wheat_beep_you_up.mp3` raw and
  ...
  Archived (not deleted) at `assets/voice/022_archive_superseded_lines/`, out
  of both `assets/voice/022/` and `assets/voice/022_processed_cartoon/`, so
  they cannot accidentally be picked up by the final mix.
