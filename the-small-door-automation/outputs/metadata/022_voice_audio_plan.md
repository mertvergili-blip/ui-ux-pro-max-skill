...
| 4 | 11.8s | 3 | wheat_bread | "I'll [bleep] you up." | shrill escalation; **censor required** |
...
- **Audio**: the literal swear word is never sent to or generated by the TTS
  engine — `generate_voice_022.py` sends the text with a literal "beep"
  placeholder in that word's position (e.g. `"I'll beep you up."`,
...
- **Caption**: rendered censored on screen, matching the channel convention
  — `"I'll f*** you up."` and `"Breakfast is over, b****."` — never spelled
...
- 11.8s wheat (censored escalation line) — rises into the bleep, never
  clips.
