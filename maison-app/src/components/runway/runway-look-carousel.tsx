...
type Slide =
  | { kind: "photo"; id: string; dataUrl: string; designer: string; season: string }
  | {
      kind: "generative";
      id: string;
      designer: string;
...
export function RunwayLookCarousel() {
  ...
  const slides = useMemo<Slide[]>(() => {
    ...
    return RUNWAY_LOOKS.map((l) => ({ kind: "generative" as const, ...l }));
  }, [photos]);

  const [rawIndex, setIndex] = useState(0);
  const [paused, setPaused] = useState(false);
  // Slide count can change (photo added/removed) — clamp at render time
      ...
      <AnimatePresence mode="wait">
        {slide.kind === "photo" ? (
          ...
          <motion.div
            ...
            style={{
              background: `
                radial-gradient(ellipse 60% 85% at 50% 42%, color-mix(in srgb, ${slide.palette[0]} 22%, transparent), transparent 70%),
                linear-gradient(175deg, color-mix(in srgb, ${slide.palette[1]} 88%, var(--color-ink)) 0%, var(--color-ink) 75%)
              `,
            }}
          >
            ...
            <motion.div
              ...
              style={{
                background: `linear-gradient(180deg, color-mix(in srgb, ${slide.palette[0]} 55%, transparent) 0%, transparent 85%)`,
                filter: "blur(1px)",
              }}
            />
            <div
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {slide.kind === "photo" ? (
              ...
              <>
                <p className="font-serif text-xl italic text-bone">{slide.mood}</p>
                <p className="mt-1.5 text-[10.5px] uppercase tracking-[1.5px] text-muted">
                  Look {String(slide.lookNumber).padStart(2, "0")} /{" "}
                  {String(slide.totalLooks).padStart(2, "0")}
                </p>
              </>
            )}
          </motion.div>
